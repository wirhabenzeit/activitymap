...
export class SelectionControl {
  constructor(layers, source, selectionHandler) {
    ...
    this.selectedFeatureIDs = url.searchParams.has("selected")
      ? url.searchParams
          .get("selected")
          .split(",")
          .map((id) => parseInt(id))
  ...
  onAdd(map) {
    ...
    map.on("click", (e) => {
      ...
      this.selectedFeatures = map.queryRenderedFeatures(bbox, {
        layers: this.layers,
      });
      this.postSelection();
    });
  ...
  postSelection = () => {
    ...
    this.selectedFeatures.forEach((feature) => {
      if (this.selectedFeatureIDs.includes(feature.properties.id)) {
        ...
        this._map.setFeatureState(
          { source: this.source, id: feature.id },
          { selected: true }
        );
      }
    });
