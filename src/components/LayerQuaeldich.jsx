...
export function LayerQuaeldich({ bbox, mapRef }) {
  ...
  console.log("LayerQuaeldich render");
  ...
  const onClick = async (e) => {
    ...
    console.log({ ...data, ...selectedFeatures[0].properties });
    getRoutes(data.kmls);
  };
  ...
  const getRoute = async (url, id) => {
    ...
    var regExp = /\(([^)]+)\)/;
    var matches = regExp.exec(geojson.features[0].properties.name);
    setCard((card) => ({
      ...card,
      kml_infos: {
        ...card.kml_infos,
