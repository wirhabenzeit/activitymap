...
export const LayerSAC: React.FC<CustomLayerProps> = ({
    ...
    geojson.features = geojson.features.map((feature) => ({
      ...
      geometry: {
        ...
        coordinates: feature.geometry.coordinates.map(
          (coord) => proj4(epsgCH, "EPSG:4326", coord)
        ),
      },
    }));
    console.log(
      "Fetched " + geojson.features.length + " SAC routes"
    );
    setJson(geojson);
  }
  ...
  useEffect(() => {
    ...
    if (
      bboxArea < 0.02 ||
      mapRef.current.getMap().getZoom() > 12.5
    )
      fetchGeoJSON();
                ...
                cards[activeStep].photos.length > 0 && (
                  ...
                  >
                    <Box
                      component="img"
                      src={
                        cards[activeStep].photos[
                          activePhoto
                        ].photo.thumbnails["320x200"]
                      }
                      alt={
                        cards[activeStep].photos[
                          activePhoto
                    ...
                    <IconButton
                      ...
                      disabled={
                        activePhoto ===
                        cards[activeStep].photos.length - 1
                      }
                      onClick={() =>
                        setActivePhoto(activePhoto + 1)
                      }
              ...
              <CardContent>
                ...
                {cards[activeStep] && (
                  <Typography variant="h5" component="div">
                    {
                      cards[activeStep].destination_poi
                        .display_name
                    }
                  </Typography>
                )}
                {cards[activeStep] && (
                  <>
                    ...
                    <Chip
                      icon={<HikingIcon />}
                      label={
                        cards[activeStep].main_difficulty
                      }
                      size="small"
                      sx={{mx: 0.5}}
                    />
