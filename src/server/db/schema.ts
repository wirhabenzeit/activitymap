...
export const accounts = pgTable(
  'account',
  {
    ...
    providerAccountId: integer('providerAccountId').primaryKey().unique(),
...
export type SportType = (typeof sportTypeEnum.enumValues)[number];
...
const valueColumns = {
  ...
  start_date_local_timestamp: bigint('start_date_local_timestamp', {
    mode: 'number',
  }).notNull(),
  ...
  max_watts: real('max_watts'),
  average_watts: real('average_watts'),
  weighted_average_watts: real('weighted_average_watts'),
...
export const photos = pgTable('photos', {
  ...
  urls: json('urls').$type<Record<number, string>>().notNull(),
  sizes: json('sizes').$type<Record<number, [number, number]>>().notNull(),
