  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const sendMessage = (message: AssistantMessage) => {
        controller.enqueue(
          textEncoder.encode(
            formatStreamPart("assistant_message", message)
          )
        );
      };
      ...
      const sendDataMessage = (message: DataMessage) => {
        controller.enqueue(
          textEncoder.encode(
            formatStreamPart("data_message", message)
          )
        );
      };
      ...
      const sendError = (errorMessage: string) => {
        controller.enqueue(
          textEncoder.encode(
            formatStreamPart("error", errorMessage)
          )
        );
      };
        ...
        for await (const value of stream) {
          switch (value.event) {
            ...
            case "thread.run.step.delta": {
              const content =
                value.data.step_details.tool_calls[0];

              if (
                content?.type === "code_interpreter" &&
                content.code_interpreter?.input != null
              ) {
                controller.enqueue(
                  textEncoder.encode(
                    formatStreamPart(
                      "text",
                      content.code_interpreter.input
                    )
                  )
                );
              }
...
/**
   ...
   */
export const experimental_AssistantResponse =
  AssistantResponse;
