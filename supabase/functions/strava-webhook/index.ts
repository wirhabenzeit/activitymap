...
function simplify_activity(activity) {
  ...
  if ("map" in activity) {
    if ("polyline" in activity["map"]) {
      ...
      coordinates = coordinates.map((coordinate: number[]) => [
        coordinate[1],
        coordinate[0],
      ]);
      simplified_activity["geometry"] = {
        type: "LineString",
        coordinates: coordinates,
      };
      var coordinates_dict = coordinates.map((coordinate: number[]) => {
        return { x: coordinate[0], y: coordinate[1] };
      });
      var simplified_coordinates = simplify(coordinates_dict, 0.0001, true);
      simplified_activity["geometry_simplified"] = {
        type: "LineString",
        coordinates: simplified_coordinates.map(
          (coordinate: { x: number; y: number }) => [coordinate.x, coordinate.y]
        ),
      };
    } else {
...
serve(async (req: Request) => {
  ...
  if (
    ...
    "object_id" in args &&
    (args["aspect_type"] == "update" || args["aspect_type"] == "create")
  ) {
    ...
    const { data2, erro2 } = await supabaseClient
      .from("strava-activities")
      .upsert(simplified_activity, {
        onConflict: "id",
        ignoreDuplicates: false,
      });
    console.log(data2, erro2);
    return new Response(JSON.stringify(simplified_activity), {
      headers: {
    ...
    const { data2, erro2 } = await supabaseClient
      .from("strava-activities")
      .upsert(simplified_activities, {
        onConflict: "id",
        ignoreDuplicates: false,
      });
    console.log(data2, erro2);
    return new Response(JSON.stringify(simplified_activities), {
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
      },
    ...
    return new Response(JSON.stringify({ Status: "Deleted", id: id }), {
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
      },
    ...
    return new Response(JSON.stringify({ Status: "Unknown request" }), {
      headers: {
        "Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*",
      },
